...
if (!file) {
  console.error('usage: node tools/mapPreview.ts <map.json> [--camp x,y] [--step n]');
  process.exit(1);
}
...
for (const s of authored.starts) {
  for (let dy = 0; dy < 3; dy++) {
    for (let dx = 0; dx < 3; dx++) marks.set(tileIdx(s.x + dx, s.y + dy, map.size), 'C');
  }
}
...
if (camp) {
  ...
  for (let dy = 0; dy < 3; dy++) {
    for (let dx = 0; dx < 3; dx++) marks.set(tileIdx(cx + dx, cy + dy, map.size), 'B');
  }
}
...
console.log(`${authored.name} — ${map.play}x${map.play} playable, ${authored.players} seat(s)`);
