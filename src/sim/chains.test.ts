...
describe('gatherer placement', () => {
  it('requires a matching deposit nearby', () => {
    const world = bareWorld();
    expect(canPlace(world.map, BuildingTypeId.ironMine, 30, 30)).toBe(false);
  ...
  it('refuses a woodcutter with no trees in reach, and a quarry with no rock', () => {
    ...
    expect(canPlace(world.map, BuildingTypeId.quarry, 30, 30)).toBe(false);
    ...
    expect(canPlace(world.map, BuildingTypeId.quarry, 30, 30)).toBe(false); // trees aren't rock
  ...
  it('the placed hut actually finds the wood the rule promised', () => {
    ...
    const hut = placeBuiltBuilding(world, BuildingTypeId.woodcutter, 0, 30, 30);
