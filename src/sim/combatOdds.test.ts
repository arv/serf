...
function only(cls: 'heavy' | 'light' | 'ranged', n: number): Force {
  const base: Force = { heavy: 0, light: 0, ranged: 0, hp: n * classHp(cls) };
  return { ...base, [cls]: n };
}
...
describe('the military triangle, as the predictor sees it', () => {
  ...
  it('still lets numbers tell against a kiting force', () => {
    ...
    const archers = only('ranged', 7);
    expect(shouldCommit(only('heavy', 7), archers, 30)).toBe(false);
    expect(shouldCommit(only('heavy', 16), archers, 30)).toBe(true);
  });

  it('calls a mirror match a coin flip', () => {
    for (const cls of ['heavy', 'light', 'ranged'] as const) {
      const a = only(cls, 7);
  ...
  it('scales power with numbers, so eleven knights beat seven', () => {
    const few = only('heavy', 7);
    const many = only('heavy', 11);
...
describe('shouldCommit', () => {
  ...
  it('gets choosier as the appetite rises', () => {
    ...
    const mine = only('heavy', 9);
    const theirs = only('heavy', 7);
  ...
  it('reads in headcount-shaped units, not squared ones', () => {
    ...
    const share = survivingFraction(only('heavy', 14), only('heavy', 7)) * 100;
  ...
  it('never refuses more as the odds improve', () => {
    ...
    for (const odds of [10, 30, 50, 70, 90]) {
      let sawCommit = false;
      for (let n = 1; n <= 20; n++) {
        const commit = shouldCommit(only('heavy', n), theirs, odds);
...
describe('survivorsAfter', () => {
  it('reports nothing standing when the fight is lost', () => {
    expect(survivorsAfter(only('light', 7), only('heavy', 7))).toBe(0);
  });

  it('reports fewer survivors than went in, and more from a bigger edge', () => {
    const theirs = only('light', 7);
    const close = survivorsAfter(only('heavy', 8), theirs);
    const easy = survivorsAfter(only('heavy', 16), theirs);
    expect(close).toBeGreaterThan(0);
    expect(close).toBeLessThan(only('heavy', 8).hp);
    expect(easy).toBeGreaterThan(close);
  });

  it('leaves an unopposed force untouched', () => {
    const army = only('heavy', 5);
    const nobody: Force = { heavy: 0, light: 0, ranged: 0, hp: 0 };
