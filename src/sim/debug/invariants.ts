...
export function checkInvariants(world: World): InvariantReport {
  ...
  for (const u of world.units.values()) {
    ...
    if (u.jobId !== undefined) {
      ...
      // order to fight is the same: the mob a last stand calls up
      ...
      // for a raid on the way in — so a carrier under either is a seat
      // spending what it has, not a lost hand. What is still wrong is a
