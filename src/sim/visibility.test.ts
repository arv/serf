...
describe('seat vision', () => {
  it('lights ground around your own units and not around anyone else', () => {
    ...
    spawnUnit(world, UnitTypeId.serf, 0, 20.5, 20.5);
    spawnUnit(world, UnitTypeId.serf, 1, 40.5, 40.5);
  ...
  it('remembers explored ground after the unit walks away', () => {
    const world = bareWorld();
    const scout = spawnUnit(world, UnitTypeId.serf, 0, 10.5, 10.5);
    const v = new SeatVision(world.map.size);
    v.recompute(world, 0);
  ...
  it('does not light ground for a dead unit', () => {
    const world = bareWorld();
    const u = spawnUnit(world, UnitTypeId.serf, 0, 15.5, 15.5);
    u.dead = true;
    const v = new SeatVision(world.map.size);
    v.recompute(world, 0);
  ...
  it('keeps the visible grid consistent with canSee', () => {
    const world = bareWorld();
    spawnUnit(world, UnitTypeId.serf, 0, 25.5, 25.5);
    const v = new SeatVision(world.map.size);
    v.recompute(world, 0);
