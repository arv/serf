...
describe('admin sandbox', () => {
  it('toggleRaids off silences the bandit camp', () => {
    ...
    tickWorld(
      world,
      cmds({kind: CommandKind.admin, action: AdminAction.toggleRaids}),
    );
    run(world, 200);
  ...
  it('clearBandits kills every bandit on the map', () => {
    ...
    tickWorld(
      world,
      cmds({kind: CommandKind.admin, action: AdminAction.clearBandits}),
    );
    // All dead at once; the corpses linger for the death animation...
    expect(
      [...world.units.values()].filter(u => u.owner === BANDIT && !u.dead),
    ).toEqual([]);
    // ...then sweep away.
  ...
  it('grantGoods fills the storehouse and keeps the ledger honest', () => {
    ...
    tickWorld(
      world,
      cmds({kind: CommandKind.admin, action: AdminAction.grantGoods}),
    );
  ...
  it('instantBuild completes sites with no materials', () => {
    ...
    tickWorld(
      world,
      cmds({kind: CommandKind.admin, action: AdminAction.toggleInstantBuild}),
    );
  ...
  it('spawnParade lines up one of each unit kind, all player-owned', () => {
    ...
    tickWorld(
      world,
      cmds({kind: CommandKind.admin, action: AdminAction.spawnParade}),
    );
