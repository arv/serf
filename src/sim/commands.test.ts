...
describe('command screening', () => {
  it('accepts what the client actually sends', () => {
    const cases: SimCommand[] = [
      ...
      { kind: CommandKind.moveUnits, unitIds: [1, 2, 3], x: 10, y: 12, attack: true },
      { kind: CommandKind.moveUnits, unitIds: [1, 2, 3], x: 10, y: 12, attack: 'half' },
      { kind: CommandKind.placeBuilding, building: BuildingTypeId.woodcutter, x: 4, y: 5 },
      ...
      { kind: CommandKind.cancelTraining, buildingId: 7, index: 2, unit: UnitTypeId.spearman },
  ...
  it('rejects names that are not defs', () => {
    expect(sanitizeCommand({ kind: CommandKind.placeBuilding, building: 'bogus', x: 1, y: 1 })).toBeNull();
    expect(sanitizeCommand({ kind: 'research', tech: 'bogus' })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.trainUnit, buildingId: 1, unit: 'bogus' })).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.cancelTraining, buildingId: 1, index: 0, unit: 'bogus' }),
    ).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.cancelTraining, buildingId: 1, index: -1, unit: 'spearman' }),
    ).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.cancelTraining, buildingId: 1, index: 1.5, unit: 'spearman' }),
    ).toBeNull();
    ...
    expect(
      sanitizeCommand({ kind: CommandKind.placeBuilding, building: 'constructor', x: 1, y: 1 }),
    ).toBeNull();
  });

  it('rejects missing and wrong-typed fields', () => {
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, x: 1, y: 1 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: 5, x: 1, y: 1 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: ['a'], x: 1, y: 1 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: [1.5], x: 1, y: 1 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: [1], x: NaN, y: 1 })).toBeNull();
    // A garbled attack flag degrades to the order that starts no fights.
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: [1], x: 1, y: 1, attack: 'yes' })).toEqual({
      kind: CommandKind.moveUnits,
      unitIds: [1],
      x: 1,
      y: 1,
    });
    expect(sanitizeCommand({ kind: CommandKind.placeBuilding, building: BuildingTypeId.woodcutter })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.setBuildingRepair, buildingId: 'x', repair: true })).toBeNull();
    // A half-given rally pair is garbage, not a guess at what was meant;
    // only the fully absent pair reads as "take the flag down".
    expect(sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 3, x: 10 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 3, y: 10 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 3, x: 1.5, y: 2 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 3, x: NaN, y: 2 })).toBeNull();
    expect(sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 'x', x: 1, y: 2 })).toBeNull();
    // A garbled flag reads as the cancel — of the two readings, the one
    // that spends nothing.
    expect(sanitizeCommand({ kind: CommandKind.setBuildingRepair, buildingId: 3, repair: 'yes' })).toEqual({
  ...
  it('bounds a queue slot by the queue that holds it', () => {
    ...
    const forge = (index: number) =>
      sanitizeCommand({ kind: CommandKind.cancelForge, buildingId: 1, index, recipeIndex: 0 });
    ...
    const train = (index: number) =>
      sanitizeCommand({ kind: CommandKind.cancelTraining, buildingId: 1, index, unit: UnitTypeId.spearman });
  ...
  it('bounds a recipe index by the longest forge menu, and lets auto through', () => {
    ...
    const enqueue = (recipeIndex: number) =>
      sanitizeCommand({ kind: CommandKind.enqueueForge, buildingId: 1, recipeIndex });
    ...
    const recipe = (index: number) =>
      sanitizeCommand({ kind: CommandKind.setBuildingRecipe, buildingId: 1, index });
  ...
  it('caps the unit list and the frame', () => {
    ...
    expect(sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: tooMany, x: 1, y: 1 })).toBeNull();

    const frame = Array.from({ length: 50 }, () => ({ kind: CommandKind.hireSerf }));
    ...
    expect(sanitizeCommands([null, { kind: CommandKind.hireSerf }, 'x'], 8)).toEqual([{ kind: CommandKind.hireSerf }]);
  });
