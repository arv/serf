...
describe('command screening', () => {
  ...
  it('rejects names that are not defs', () => {
    expect(
      sanitizeCommand({ kind: CommandKind.placeBuilding, building: 'bogus', x: 1, y: 1 }),
    ).toBeNull();
    expect(sanitizeCommand({ kind: 'research', tech: 'bogus' })).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.trainUnit, buildingId: 1, unit: 'bogus' }),
    ).toBeNull();
    expect(
  ...
  it('rejects missing and wrong-typed fields', () => {
    ...
    expect(
      sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: [1], x: 1, y: 1, attack: 'yes' }),
    ).toEqual({
    ...
    expect(
      sanitizeCommand({ kind: CommandKind.placeBuilding, building: BuildingTypeId.woodcutter }),
    ).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.setBuildingRepair, buildingId: 'x', repair: true }),
    ).toBeNull();
    ...
    expect(
      sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 3, x: 1.5, y: 2 }),
    ).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 3, x: NaN, y: 2 }),
    ).toBeNull();
    expect(
      sanitizeCommand({ kind: CommandKind.setRallyPoint, buildingId: 'x', x: 1, y: 2 }),
    ).toBeNull();
    // A garbled flag reads as the cancel — of the two readings, the one
    // that spends nothing.
    expect(
      sanitizeCommand({ kind: CommandKind.setBuildingRepair, buildingId: 3, repair: 'yes' }),
    ).toEqual({
  ...
  it('caps the unit list and the frame', () => {
    ...
    expect(
      sanitizeCommand({ kind: CommandKind.moveUnits, unitIds: tooMany, x: 1, y: 1 }),
    ).toBeNull();
    ...
    expect(sanitizeCommands([null, { kind: CommandKind.hireSerf }, 'x'], 8)).toEqual([
      { kind: CommandKind.hireSerf },
    ]);
  });
