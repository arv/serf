...
function sampleState() {
  ...
  applyBrush(state, {kind: 'resource', res: TileResource.Wood}, 62, 62, {
    radius: 3,
    folds: 4,
  });
...
describe('map file round-trip', () => {
  it('preserves everything an authored map is', () => {
    ...
    for (let i = 0; i < state.map.height.length; i++) {
      expect(
        Math.abs(back.map.height[i]! - state.map.height[i]!),
...
describe('map file validation', () => {
  ...
  it('rejects garbage and wrong tags', () => {
    ...
    expect(() =>
      parseEditorMap(fileOf(f => ((f as {version: number}).version = 3))),
    ).toThrow(/version/);
  });
  ...
  it('rejects bad sizes', () => {
    for (const play of [63, 62, 130, 65.5]) {
      expect(() =>
        parseEditorMap(fileOf(f => ((f as {play: number}).play = play))),
      ).toThrow(/playable size/);
    }
    // The grid must be exactly gridFor(play) — the canonical margin.
    expect(() =>
      parseEditorMap(fileOf(f => ((f as {size: number}).size = 64))),
    ).toThrow(/grid size/);
  });

  it('rejects short grids and out-of-range values', () => {
    expect(() => parseEditorMap(fileOf(f => dropTile(f, 'terrain')))).toThrow(
      /terrain/,
    );
    expect(() =>
      parseEditorMap(fileOf(f => (f.height = f.height.slice(4)))),
    ).toThrow(/height/);
    expect(() => parseEditorMap(fileOf(f => poke(f, 'terrain', 0, 7)))).toThrow(
      /terrain value/,
    );
    expect(() =>
      parseEditorMap(fileOf(f => poke(f, 'resource', 0, 9))),
    ).toThrow(/resource value/);
    // Well inside what an int16 of millimetres holds, well outside a world.
    expect(() => parseEditorMap(fileOf(f => pokeHeight(f, 0, 9)))).toThrow(
      /height/,
    );
  });
  ...
  it('rejects a grid that is not base64 at all', () => {
    expect(() =>
      parseEditorMap(fileOf(f => (f.terrain = '~~ not base64 ~~'))),
    ).toThrow(/terrain is not base64/);
  });
  ...
  it('rejects starts that do not fit the seats or the play area', () => {
    ...
    expect(() =>
      parseEditorMap(fileOf(f => (f.starts[0] = {x: 10, y: 40}))),
    ).toThrow(/out of bounds/);
    expect(() =>
      parseEditorMap(fileOf(f => (f.starts[0] = {x: 94, y: 40}))),
    ).toThrow(/out of bounds/);
    expect(() =>
      parseEditorMap(fileOf(f => ((f as {players: number}).players = 5))),
    ).toThrow(/player/);
  });
});
...
describe('version 1 files', () => {
  ...
  function v1Of(mutate: (f: V1) => void = () => {}): string {
    ...
    for (let i = 0; i < bytes.length / 2; i++)
      height.push(view.getInt16(i * 2, true) / 1000);
    const old: V1 = {
      ...f,
      version: 1,
  ...
  it('are still screened for what a JSON array can hold', () => {
    ...
    expect(() => parseEditorMap(v1Of(f => (f.terrain[0] = 'grass')))).toThrow(
      /terrain/,
    );
    expect(() => parseEditorMap(v1Of(f => (f.resourceAmt[0] = 300)))).toThrow(
      /resourceAmt/,
    );
    // JSON writes a non-finite number as null; null must not read as 0.
    expect(() => parseEditorMap(v1Of(f => (f.height[0] = Infinity)))).toThrow(
      /height/,
    );
  });
});
