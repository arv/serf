...
function hrefFor(
  ...
  for (const piece of linkifyProse(text, self)) {
    if (
      typeof piece !== 'string' &&
      piece.text.toLowerCase() === term.toLowerCase()
    ) {
      return piece.href;
    }
  }
...
describe('prose linking', () => {
  it('links the things a sentence names', () => {
    ...
    expect(hrefFor('Trained at the Barracks.', 'Barracks')).toBe(
      '/docs/buildings/barracks',
    );
    expect(hrefFor('Kites knights all day.', 'knights')).toBe(
      '/docs/units/knight',
    );
    expect(hrefFor('Wants Brewing researched.', 'Brewing')).toBe(
      '/docs/techs#tech-brewing',
    );
  });

  it('follows plurals, including the old one', () => {
    // A trailing 's' covers most of them; Spearman does not take one.
    expect(hrefFor('Unlocks the Barracks and Spearmen.', 'Spearmen')).toBe(
      '/docs/units/spearman',
    );
    expect(hrefFor('Bring spears and walls.', 'spears')).toBe(
      '/docs/goods/spear',
    );
  });

  it('follows the words prose actually uses for a good', () => {
    expect(hrefFor('The head of the bread chain.', 'bread')).toBe(
      '/docs/goods/food',
    );
  ...
  it('prefers the longest name', () => {
    ...
    expect(hrefFor('Ore from the iron mine.', 'iron mine')).toBe(
      '/docs/buildings/ironMine',
    );
    expect(hrefFor('A wave of Bandit Archers.', 'Bandit Archers')).toBe(
      '/docs/units/banditArcher',
    );
  });

  it('links a term once, however often it is named', () => {
    expect(marked('Wood, more wood, and wood again.')).toBe(
      '[Wood], more wood, and wood again.',
    );
  });

  it('never links the page being read', () => {
    expect(
      hrefFor('Ale is brewed here.', 'Ale', '/docs/goods/ale'),
    ).toBeUndefined();
  });
  ...
  it('does not turn a verb into a link', () => {
    ...
    expect(
      marked('The barracks keeps a cask: each soldier drinks 1 ale.'),
    ).toBe('The [barracks] keeps a cask: each soldier drinks 1 [ale].');
  });
  ...
  it('leaves ordinary English alone', () => {
    ...
    expect(marked('well past when a rush arrives')).toBe(
      'well past when a rush arrives',
    );
    expect(marked('placed by the Masonry road pass')).toBe(
      'placed by the [Masonry] road pass',
    );
    expect(
      hrefFor('the barracks keeps a cask of festivals', 'festivals'),
    ).toBeUndefined();
  });

  it('links an ambiguous word the sentence marks as a name', () => {
    // Either a capital of its own...
    expect(hrefFor('Water is drawn at the Well.', 'Well')).toBe(
      '/docs/buildings/well',
    );
    ...
    expect(hrefFor('one mill is meant to serve two', 'mill')).toBe(
      '/docs/buildings/mill',
    );
  ...
  it('follows the short name a sentence gives a building', () => {
    expect(hrefFor('the farm all start here', 'farm')).toBe(
  ...
  it('does not read a leading capital as a proper noun', () => {
    ...
    expect(
      hrefFor('Nothing goes in. Food comes out slowly.', 'Food'),
    ).toBeUndefined();
    expect(
      hrefFor('One hut, one hand. Well water is drawn here.', 'Well'),
    ).toBeUndefined();
  });
});
