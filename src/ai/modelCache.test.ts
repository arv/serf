...
const HF_URL = 'https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct-GGUF/resolve/main/model.gguf';
const SIZE = 4096;
...
function serveModel(): void {
  vi.stubGlobal('fetch', (input: string, init?: { method?: string }) => {
    ...
    if (url.includes('/raw/')) {
      return Promise.resolve(new Response(`oid sha256:${'a'.repeat(64)}\nsize ${SIZE}\n`));
    }
    const headers = { 'content-length': String(SIZE), etag: '"v1"' };
    if (init?.method === 'HEAD') return Promise.resolve(new Response(null, { headers }));
    return Promise.resolve(new Response(new Uint8Array(SIZE) as BodyInit, { headers }));
  });
}
...
describe('discardPartialModel', () => {
  ...
  function fakeCache(entries: ModelCacheEntry[]): ModelCache & { deleted: string[] } {
  ...
  it('leaves other models alone, whole or broken', async () => {
    ...
    const cache = fakeCache([
      entry({ name: 'key(other)', metadata: { originalURL: other, originalSize: SIZE } }),
      entry({ name: 'key(other-partial)', metadata: { originalURL: '', originalSize: SIZE } }),
    ]);
    expect(await discardPartialModel(cache, URL_)).toBe(0);
    expect(cache.deleted).toEqual([]);
  });
...
describe('the wllama cache, as the sweep finds it', () => {
  function manager(): { mm: ModelManager; backend: ReturnType<typeof memoryBackend> } {
  ...
  it('fails forever on the leftovers of an interrupted download', async () => {
    ...
    await expect(mm.downloadModel(HF_URL)).rejects.toThrow(`Model file not found: ${HF_URL}`);
