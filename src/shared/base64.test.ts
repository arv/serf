...
describe('base64 typed arrays', () => {
  ...
  it('round-trips float32 exactly — a save resumes on these numbers', () => {
    const values = Float32Array.of(0, -0, 1 / 3, -1e-8, 3.4e38, Math.fround(0.30000001192092896));
  ...
  it('is little-endian on the wire, whatever the host is', () => {
    ...
    expect(int16ToBase64(Int16Array.of(0x0102))).toBe(bytesToBase64(Uint8Array.of(0x02, 0x01)));
  ...
  it('drops a trailing partial element rather than guessing', () => {
    expect(int16FromBase64(bytesToBase64(Uint8Array.of(1, 0, 9))).length).toBe(1);
    expect(float32FromBase64(bytesToBase64(Uint8Array.of(1, 2, 3))).length).toBe(0);
  });
});
