 ...
 * tools economy. Version 7 halved the scenery margin, which is the same
 * kind of change as 4: the grid an older file describes is not a grid this
 * build makes. Older saves are refused rather than silently mis-loaded.
 *
 ...
 * send. Seven: the scenery margin is a quarter of the playable side, so a
 * six's arrays describe a grid twice as wide as anything this build lays
 * out — the widest of them do not even fit the ceiling a file is allowed,
 * and the rest would load a valley framed by a ring no editor here can
 * paint and no seed here can roll. Either way the village loads into
 * something it cannot explain, which is worse than being turned away.
 *
