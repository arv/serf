...
describe("the fishery's pier", () => {
  it('reports a deck line turned with the building, landward end first', () => {
    ...
    const p = piers[0]!;
    expect(p.bx).toBeCloseTo(11);
    expect(p.bz).toBeCloseTo(11);
    ...
    expect(p.baseZ).toBeLessThan(11);
    expect(p.spotZ).toBeLessThan(p.baseZ);
    expect(p.baseX).toBeCloseTo(11);
    expect(p.spotX).toBeCloseTo(11);
  ...
  it('leaves the authored deck alone when it already reaches water', () => {
    // Facing 2 sends the deck north out of the 2x2 at (10,10): landward
    // end at z 10.15, tip at 7.65, fishing spot at 8.05. Open water from
    // z 9 north, so the authored placement stands over it.
    const {sync, scene} = makeSync(shoreHeights((_tx, tz) => tz <= 8));
    sync.update([snap({type: BuildingTypeId.fishery, facing: 2})]);
    const p = sync.fisheryPiers()[0]!;
    expect(p.yaw).toBeCloseTo(Math.PI);
    expect(p.spotX).toBeCloseTo(11);
    expect(p.spotZ).toBeCloseTo(8.05);
  ...
  it('trims the deck rather than stride over a narrow channel', () => {
    // The single tile row at tz 8, straight off the front face, so the wet
    // band runs from world z 8 to z 9: the authored deck reaches z 7.65,
    ...
    sync.update([snap({type: BuildingTypeId.fishery, facing: 2})]);
    const p = sync.fisheryPiers()[0]!;
    // Still pointing where the hut faces — half a tile shorter instead.
    expect(p.yaw).toBeCloseTo(Math.PI);
    expect(p.baseZ).toBeCloseTo(10.15);
    expect(p.spotZ).toBeCloseTo(8.55);
    const pier = scene.getObjectByName('fisheryPier')!;
    expect(pier.rotation.y).toBeCloseTo(0);
    expect(pier.scale.x).toBeCloseTo(0.8);
    // A smaller dock, planks and all, so the standing height comes down.
    expect(p.deckY).toBeCloseTo(0.14);
  });
...
describe('the damage bars', () => {
  ...
  it('turn with the camera, not only when a building changes', () => {
    ...
    const cam = new THREE.Quaternion().setFromAxisAngle(
  ...
  it('is not fooled into rebuilding by a pan', () => {
    ...
    const tiny = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(0, 1, 0),
      1e-3,
    );
    cam.multiply(tiny);
...
describe("the fishery's shoal", () => {
  it('swims under the waterline, not at the deck height the template bakes', () => {
    const {sync, scene} = makeSync();
    sync.update([
      snap({type: BuildingTypeId.fishery, staffing: StaffingState.staffed}),
    ]);
  ...
  it('swims nose-first, whichever way round its circle it goes', () => {
    ...
    const speeds = shoal!.children.map(
      p => (p.userData as {speed: number}).speed,
    );
...
describe('the stock piles at a building door', () => {
  ...
  function stackXs(scene: THREE.Scene, h = 3): number[] {
    ...
    const piles = root.children.find(
      o =>
        ...
        Math.abs(o.position.z - (h / 2 + 0.3)) < 1e-6,
    ) as THREE.Group | undefined;
    if (!piles) return [];
    const clusters: number[][] = [];
    for (const prop of [...piles.children].sort(
      (a, b) => a.position.x - b.position.x,
    )) {
      const last = clusters[clusters.length - 1];
      if (last && Math.abs(prop.position.x - last[0]!) < 0.1)
        last.push(prop.position.x);
  ...
  it('leaves the stacks already standing where they are when a new good arrives', () => {
    ...
    sync.update([
      castle({[GoodId.wood]: 4, [GoodId.stone]: 2, [GoodId.iron]: 1}),
    ]);
  ...
  it('hands a drained good its lane back for the next arrival', () => {
    ...
    const refilled =
      (sync.update([castle({[GoodId.stone]: 2, [GoodId.iron]: 3})]),
      stackXs(scene));
