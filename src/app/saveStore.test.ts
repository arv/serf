...
describe('the saves shelf', () => {
  it('files each save as its own document and hands them back newest first', async () => {
    installOpfs();
    expect(await saveGameFile('2026-01-01 09.00.00', save())).toBe('2026-01-01 09.00.00');
    expect(await saveGameFile('2026-01-02 09.00.00', save())).toBe('2026-01-02 09.00.00');
    const found = await listSaveFiles();
    expect(found.map((f) => f.name)).toEqual(['2026-01-02 09.00.00', '2026-01-01 09.00.00']);
  ...
  it('lists a save from before the metadata head with nothing to say', async () => {
    ...
    opfs.put('saves', 'ancient.json', JSON.stringify({ version: 4, world: { tick: 1 } }));
  ...
  it('files an imported save and refuses a file that is not one', async () => {
    ...
    expect(await importSaveFile(new File(['{"replayVersion":11}'], 'log.json'))).toEqual({
      ok: false,
      reason: 'unrecognized',
    });
    expect(await importSaveFile(new File(['not json at all'], 'x.json'))).toEqual({
      ok: false,
      reason: 'unrecognized',
    });
  });
