...
function playerScript(tick: number): SimCommand[] {
  ...
  if (tick === 120)
    return [{kind: CommandKind.moveUnits, unitIds: [7, 8], x: 20, y: 20}];
  if (tick === 121)
    return [{kind: CommandKind.moveUnits, unitIds: [7], x: 30, y: 12}];
  return [];
}
  ...
  it('the recorded log reproduces the live match without ever running the AI', () => {
    ...
    for (let t = 0; t < 1500; t++) {
      ...
      if (executed.length > 0)
        log.push({tick: live.tick, commands: executed.slice()});
    ...
    for (let t = 0; t < 1500; t++) {
      ...
      while (
        cursor < parsed.commands.length &&
        parsed.commands[cursor]!.tick === replayed.tick
      ) {
